...
int main(int argc, char **argv)
{
    MPI_Init(&argc, &argv);
    {
       ...
       /*
        * Different from the prior version, we're NOT using HashMap, Bloom Filter, or HyperLogLog currently
        * 
        * We are now using sorting algorithm (specifically Parallized Radix Sort, or PARADIS) to complete the task.
        * Time performance is better, and OpenMP implementation is supported with this approach.
        * 
        * Note that we may take up more memory with this approach. There's room for improvement, but 
        * it may make the code look ugly.
        * 
